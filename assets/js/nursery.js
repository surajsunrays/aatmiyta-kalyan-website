...
const slideTransitions = [
  {
    enter: "translateX(12%) translateZ(-140px) rotateY(-16deg) scale(0.92)",
    exit: "translateX(-10%) translateZ(-100px) rotateY(12deg) scale(0.94)",
  },
  {
    enter: "translateX(-12%) translateZ(-140px) rotateY(16deg) scale(0.92)",
    exit: "translateX(10%) translateZ(-100px) rotateY(-12deg) scale(0.94)",
  ...
  {
    enter: "translateY(12%) translateZ(-120px) rotateX(14deg)",
    exit: "translateY(-10%) translateZ(-100px) rotateX(-12deg)",
  },
  {
    enter: "translateZ(-180px) scale(0.88)",
    exit: "translateZ(60px) scale(1.06)",
  },
  {
    enter: "translateZ(-100px) rotateZ(-4deg) scale(0.94)",
    exit: "translateZ(-80px) rotateZ(4deg) scale(0.96)",
  },
];
let lastTransition = -1;
function chooseTransition() {
  // Choose a different effect each time, without changing the photo order.
  const choices = slideTransitions
    .map((_, index) => index)
...
function scheduleSlide(delay = 3500) {
  ...
  if (
    ...
    !document.hidden &&
    !viewer.open &&
    !slider.matches(":hover, :focus-within")
  ) {
...
if (sliderPhotos.length) {
  ...
  new MutationObserver(scheduleSlide).observe(viewer, {
    attributes: true,
  ...
  sliderStage.addEventListener("touchcancel", () => {
    touchStart = null;
  });
}
